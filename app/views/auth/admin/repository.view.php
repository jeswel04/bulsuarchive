...
<div class="container">
    ...
    <h1 style="margin-top: 1vh;">Repository</h1>
       <button class="field_btn " type="submit" id="filter" name="filter">Filter</button>
      <form method="GET">
        <input type="text" name="input_search" value="<?php if (isset($_GET['input_search'])){ echo $_GET['input_search'];}?>" placeholder="Search...">
        <span class="icon">
            <button type="submit" id="search" style="background-color:transparent; cursor:pointer;"><i class="uil uil-search search-icon"></i></button>
        </span>
      </form>

    <?php if(!empty(isset($_GET['input_search'])) && !empty($search)):?>
        <div class="cardContainer">
            <?php foreach($search as $keys=>$value):?>
            <div class="cardUpload">
              <a href="<?=ROOT?>/file?id=<?=$search[$keys]['research_token']?>">
              ...
              <p><?=$search[$keys]['research_description']?></p>
    ...
    <?php if(isset($_POST['filter'])):?>
        <?php if(!empty($filter)):?>
          <div class="cardContainer">
            <?php foreach($filter as $keys=>$value):?>
            <div class="cardUpload">
              <a href="<?=ROOT?>/file?id=<?=$filter[$keys]['research_token']?>">
              <h2 style="color: var(--color-primary) ;"><?=$filter[$keys]['title']?></h2>
              <span><?=$filter[$keys]['researchers']?></span>
              <br><span><?=$filter[$keys]['adviser']?></span>
              <?php if(!empty($course)):?>
                  <?php foreach($course as $key=>$value):?>
                    <?php if($filter[$keys]['course_id'] == $course[$key]['id']):?>
                      <p><?=$course[$key]['course_code']?> - <?=$course[$key]['course_description']?></p>
                    <?php endif;?>
                  <?php endforeach;?>
              <?php endif;?>
              <p><?=$filter[$keys]['research_description']?></p>
              <span><?=$filter[$keys]['date_publish']?></span>
              <h3> VIEWS:<?=$filter[$keys]['views']?> | DOWNLOADS: <?=$filter[$keys]['downloads']?></h3>
              </a>
            </div>
            <?php endforeach;?>
        </div>
    ...
    <div class="cardContainer">
      <?php if(!isset($_POST['filter'])):?>
      <?php if(!isset($_GET['input_search'])):?>
      <?php if(!empty($research)):?>
          <?php foreach($research as $key =>$value):?>
              <div class="cardUpload">
                <form method="POST">
                    <input type="text" id="saved_token" name="saved_token" value="<?=$research[$key]['research_token']?>" hidden>
                    <input type="text" id="research_id" name="research_id" value="<?=$research[$key]['id']?>" hidden>
                    <button class="btn_download" type="submit" id="saved" name="saved" style="background-color: transparent;"><span class="material-symbols-sharp" style="color:var(--color-info-dark); cursor:pointer;">bookmark</span></button>
                </form>
                <a href="<?=ROOT?>/file?id=<?=$research[$key]['research_token']?>">
                  ...
                  <br><p><?=$research[$key]['research_description']?></p>
                  <span><?=$research[$key]['date_research']?></span>
  ...
  <div class="right">
    ...
    <?php if(Auth::is_Admin()):?>
      ...
      <!--MODAL FILTER-->
      <div class="modal-container" id="modal_container_filter">
        <div class="courseList">
        <form method="POST">
        <strong><span>Course</span></strong>
        <select name="course_id" id="course_id">
            <?php if(!empty($department)):?>
              <?php foreach($department as $key=>$value):?>
                <option value="" selected disabled style="font-weight:700 ;"><?=$department[$key]['department_code']?></option>
                  <?php if(!empty($course)):?>
                        <?php foreach($course as $keys=>$value):?>
                            <?php if($department[$key]['id']==$course[$keys]['department_id']):?>
                                <option value="<?=$course[$key]['id']?>"><?=$course[$keys]['course_description']?></option>
          ...
          <?php if(!empty($year)):?>
              <?php foreach($year as $key=>$value):?>
                <?php $years = explode('-',$year[$key]['date_research']);?>
          ...
          <button class="field_btn" type="submit" name="filter" id="filter">Apply Filter</button>
          </form>
        </div>
      </div>
