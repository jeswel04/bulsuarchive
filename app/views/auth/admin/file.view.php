...
<div class="container">
  ...
  <main>
    ...
    <?php if(!empty($research)):?>
        <?php foreach($research as $key=>$title):?>
          
            <?php if($research[$key]['research_token']==$_GET['id']):?>
                <a href="<?=ROOT."/download?file=".$research[$key]['picture']?>" class="btn_download" style="background-color:var(--color-primary)">
                Download Whole Paper
                </a>
                <a href="<?=ROOT."/download?file=".$research[$key]['picture1']?>" class="btn_download" style="background-color:var(--color-warning)">
                Download Abstract
                </a>
                <div class="cardContainer">
                <div>
                <h1 style="color:var(--color-primary);"><?=$research[$key]['title']?></h1>
                <span><?=$research[$key]['researchers']?></span><br>
                <?php if(!empty($department)):?>
                    <?php foreach($department as $keys=>$value):?>
                      <?php if($research[$key]['department_id'] == $department[$keys]['id']):?>
                        <span><?=$department[$keys]['department_code']?> - <?=$department[$keys]['department_description']?></span><br>
                      <?php endif;?>
                  <?php endforeach;?>
                <?php endif;?>
                ...
                <?php foreach($course as $keys=>$value):?>
                  <?php if($research[$key]['course_id'] == $course[$keys]['id']):?>
                    <span><?=$course[$keys]['course_code']?> - <?=$course[$keys]['course_description']?></span><br>
                    <?php endif;?>
                  <?php endforeach;?>
                <?php endif;?>
                <span><?=$research[$key]['adviser']?></span><br>
                <span><?=$research[$key]['date_research']?></span>
                <p><?=$research[$key]['research_description']?></p>
                </div>
                
                    <div class="">
                      <h2>Abstract</h2>
                      <iframe src="../upload_research/<?=$research[$key]['picture1']?>#toolbar=0" frameborder="0" height="100%" width="150%"></iframe>
                      <h2>Whole Paper</h2>
                      <iframe src="../upload_research/<?=$research[$key]['picture']?>#toolbar=0" frameborder="0" height="100%" width="150%"></iframe>
                    </div>
                </div>
            <?php endif;?>
        <?php endforeach;?>
  ...
  <div class="citation">
    ...
    <h2 style="margin-top:10px ; margin-bottom:10px;">Citation</h2>
      <?php $author=explode(' ',$research[$key]['researchers'])?>
      <?php $year = explode('-', $research[$key]['date_research'])?>
