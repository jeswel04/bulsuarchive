...
<div class="container">
  ...
  <main>
    ...
    <div class="cardContainer">
      <?php if(!empty($savedpapers) && !empty($research)):?>
      <?php foreach($savedpapers as $key=>$values):?>
          <?php if($savedpapers[$key]['user_id']== $_SESSION['USER_DATA']['id']):?>
                <?php foreach($research as $keys=>$value):?>
                    <?php if($savedpapers[$key]['saved_token']==$research[$keys]['research_token']):?>
                        <div class="cardUpload">
                          <a href="<?=ROOT?>/file?id=<?=$research[$keys]['research_token']?>">
                          <h2 style="color:var(--color-primary);"><?=$research[$keys]['title']?></h2>
                          <span><?=$research[$keys]['researchers']?></span><br>
                          <span><?=$research[$keys]['department_code']?> - <?=$research[$keys]['department_description']?></span><br>
                          <span><?=$research[$keys]['course_code']?> - <?=$research[$keys]['course_description']?></span><br>
                          <span><?=$research[$keys]['adviser']?></span>
                          <p><?=$research[$keys]['research_description']?></p>
                          <p><?=$research[$keys]['date_research']?></p>
                          </a>
                        </div>
                    <?php endif;?>
                <?php endforeach;?>
