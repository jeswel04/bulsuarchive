...
<div class="container">
  ...
  <?php if(Auth::is_admin()):?>
    <div class="statistics-field">
        <div class="date">
          <form method="POST" id="formDate" name="formDate">
            <input type="date" name="date" id="date1"/>  
          </form>   
        </div>
        <form method="POST"><button type="submit" name="overAll" id="overAll">Overall Statistics</button></form> 
    </div>  
    <div class="insights">
      <div>
      <label>Uploads</label><br>
      <?php if(!empty($uploads) && !isset($_POST['date'])):?>
          <span class="notif"><?=$uploads[0]['count(*)']??0?></span>
      <?php else:?>
          <span class="notif"><?=$uploadsDate[0]['count(*)']??0?></span>
      <?php endif;?>
      </div>      
      <div>
        <label>Downloads</label><br>
        <?php if(!empty($downloads) && !isset($_POST['date'])):?>
          <span class="notif"><?=$downloads[0]['sum']??0?></span>
        <?php else:?>
          <span class="notif"><?=$downloadsDate[0]['sum']??0?></span>
        <?php endif;?>
      </div> 
      <div>
        <label>Views</label><br>
        <?php if(!empty($views)&& !isset($_POST['date'])):?>
          <span class="notif"><?=$views[0]['sum']??0?></span>
        <?php else:?>
          <span class="notif"><?=$viewsDate[0]['sum']??0?></span>
        <?php endif;?>
      </div> 
      <div>
        <label>Accounts</label><br>
        <?php if(!empty($accounts)&& !isset($_POST['date'])):?>
          <span class="notif"><?=$accounts[0]['count(*)']??0?></span>
        <?php else:?>
          <span class="notif"><?=$accountsDate[0]['count(*)']??0?></span>
        <?php endif;?>
      </div> 
    </div>
  ...
  <div class="right">
      <div class="top">
        ...
        <label class="theme-toggler-button">
          <input type="checkbox" class="theme-toggler" id="toggleTheme">
