        ...
        foreach($history as $key=>$value){
            if($history[$key]['user_id']==$id && isset($id)){
              foreach($research as $keys=>$value){
                  if($history[$key]['history_token'] == $research[$keys]['research_token']){
                    echo 
                    "<div class=\"cardUpload\">".
                    "<form method=\"POST\">".
                    "<input type=\"text\" id=\"research_token\" name=\"research_token\" value=\"".$research[$keys]['research_token']."\" hidden>".
                    "<button type =\"submit\" id=\"delete\" name=\"delete\" style=\"background-color:transparent; float:right; cursor:pointer;\"><span class=\"material-symbols-sharp\">
                    close</span></button>".
                    "<a href = \"".ROOT."/file?id=".$history[$key]['history_token']."\">".
