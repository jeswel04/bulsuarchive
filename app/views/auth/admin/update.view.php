        ...
        <div class="child">
          <?php if(!empty($tempuser)):?>
            <?php foreach ($tempuser as $key=>$value):?>
              <?php if($tempuser[$key]['temp_token']==$_GET['id']):?>
                <h3><?=$tempuser[$key]['picture']?>-<?=$tempuser[$key]['lastname']?></h3>
                <iframe id="iframe" name="iframe" src="../upload_files/<?=$tempuser[$key]['picture']?>#toolbar=0" frameborder="0"></iframe>
                </div>
                
                <form method="POST">
                    <input type = "hidden" name="user_token" id="user_token" value=<?=Auth::createToken()?>></input>
                    <h3>School Id</h3>
                    <input class="field" type="text" id="school_id" name="school_id" value="<?=$tempuser[$key]['school_id']?>" disabled>
                    <div class="grid3">
                        <div class="">
                            <h3>User Type</h3>
                            <select name="role_id" id="role_id" type="text" disabled>
                                <option value="<?=$tempuser[$key]['role_id']?>" selected="selected"><?=$tempuser[$key]['role']?></option>
                        ...
                        <div class="">
                            <h3>Department</h3>
                            <select type="text" id="department_id" name="department_id" type="text" disabled>
                                <option value="<?=$tempuser[$key]['department_id']?>" selected="select"><?=$tempuser[$key]['department_code']?></option>
                                <?php if(!empty($department)):?>
                                  <?php foreach($department as $keys=>$value):?>
                                      <option value="<?=$department[$keys]['id']?>"><?=$department[$keys]['department_code']?></option>
                                  <?php endforeach;?>
                                <?php endif;?>
                            </select>
                        </div>
                        <div class="">
                            <h3>Date Sign Up</h3>
                            <input class="field" type="date" id="date_validate" name="date_validate" value="<?=$tempuser[$key]['date_validate']?>" disabled>
                        </div>
                    </div>
                    <h3>Course</h3>
                    <select id="course_id" name="course_id" type="text" disabled>
                        <option value="<?=$tempuser[$key]['course_id']?>"><?=$tempuser[$key]['course_description']?></option>
                        <?php if(!empty($course)):?>
                          <?php foreach($course as $keys=>$value):?>
                              <option value="<?=$course[$keys]['id']?>"><?=$course[$keys]['course_description']?></option>
                          <?php endforeach;?>
                        <?php endif;?>
                    </select>
                    <h3>Last name</h3>
                    <input class="field" type="text" id="lastname" name="lastname" value="<?=$tempuser[$key]['lastname']?>" disabled>
                    <h3>First name</h3>
                    <input class="field" type="text" id="firstname" name="firstname" value="<?=$tempuser[$key]['firstname']?>" disabled>
                    <h3>BULSU email</h3>
                    <input class="field" type="text" id="email_verified" name="email_verified" value="<?=$tempuser[$key]['email']?>" disabled>
